...
Version: 0.3.0 candidate. Status: operational protocol extended to all cytokines and broader immune mechanisms; independent review of the new candidate is deferred until the user requests it; not prospectively registered in an external registry. Governing specification: README.md. Scope: human immune-system biology with separately retained animal and experimental-system evidence. Current candidate search cutoff: 2026-09-13. No previous statistical-model priors are imported.

The current phase follows [PRODUCT_SCOPE.md](PRODUCT_SCOPE.md) and [AMENDMENT_0.3.md](AMENDMENT_0.3.md). Release 0.2.0 remains the preserved baseline. New evidence and typed connection proposals are staged separately, pending review; the prior-construction and approval rules below describe the later production workflow and do not authorize scoring or approval of the current candidate.

## Review question and scope

For all cytokines and other immune-system entities, which individual genomic, transcriptional, protein, signaling and cellular relationships are supported, in which contexts, and by which independent biological studies? The unit of extraction is an assertion supported by an evidence instance. A complete communication chain is never the default extraction unit.
...
Search domains cover cytokine/signaling, genomics, transcription, proteomics and immune function. Current mechanism lanes deepen genetic regulation, activation and antigen presentation. Save exact queries, provider, retrieval time, response snapshots and screening decisions. Entity searches use canonical symbols and source-specific synonyms. Citation chaining begins at the dictionary/atlas anchors and follows relevant original experiments. DOI/PMID/accession deduplication precedes assertion weighting. Results screened for the bounded pilot or current focused searches must not be described as a PRISMA-complete retrieval. Unscreened search hits remain excluded from assertion priors.

## Normalization and molecular layers

Use distinct identifiers for genes, transcripts, proteins, protein complexes, variants, regulatory elements, cells, states, pathways, programs, processes and assay concepts. External ontology identifiers are assigned only when verified; otherwise retain a stable local ID and mark external mapping unavailable. Preserve source-specific molecular identities and any historical assay labels as provenance. Historical panels do not define current scope or research priority. Define complexes with explicit components where established.
...
Each release exports normalized CSVs, SQLite, data dictionary, corpus/search and cohort manifests, critic/adjudication tables, prior changes, QC, evidence synthesis and a SHA-256 manifest. The manifest excludes itself to avoid a circular digest. Rebuilding is deterministic from frozen inputs in a separate directory; a frozen release cannot be overwritten. Revisions belong in a new version. Candidate supplements have separate outputs and exact input snapshots and do not constitute a production freeze. No external publication is implied by a local artifact.
