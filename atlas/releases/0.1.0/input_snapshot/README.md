...
A bounded local Release 0.1.0 is now available: **72 evidence assertions**, normalized SQLite/CSV tables, source snapshots, independent review and corrections, explicit prior interfaces, and reproducible hash verification.

Start with [RUNNING.md](RUNNING.md), [the research synthesis](reports/RESEARCH_SYNTHESIS.md), and [the item-by-item project status](reports/PROJECT_STATUS.md). The latter tracks all 107 original checklist requirements and explicitly distinguishes implemented infrastructure from incomplete scientific coverage. This is not an exhaustive systematic review or a human-adjudicated, calibrated prior release. The original specification is preserved in [protocol/README.original.md](protocol/README.original.md).
