...
import sqlite3
import sys
from sanitize_snapshots import scan_files as credential_metadata_findings
...
PACK_LANES = ('cytokine','genomic','proteomic','cytokine_expansion','chemokine_expansion','receptor_expansion')
NA = 'unavailable'
...
'screening_candidates': 'candidate_id TEXT PRIMARY KEY, query_id TEXT, title TEXT, url TEXT, doi TEXT, pmid TEXT, decision TEXT, reason TEXT, lane TEXT',
'quantitative_results': 'result_id TEXT PRIMARY KEY, evidence_id TEXT REFERENCES evidence_instances, reported_result TEXT, interpretation TEXT',
'release_changes': 'change_id TEXT PRIMARY KEY, record_type TEXT, record_id TEXT, previous_version TEXT, new_version TEXT, change_type TEXT, old_record TEXT, new_record TEXT',
}
...
def source_snapshot_paths(sources):
    """Include every declared source file, including partial curation overlays."""
    paths=[]
    for source in sources:
        if 'snapshot_path' in source:
            paths.append(source['snapshot_path'])
        paths.extend(source.get('additional_snapshot_paths',[]))
    return sorted(set(paths))

def recorded_retrieval_time(path):
    rel=str(path.relative_to(ROOT))
    record=read_json(ROOT/'provenance/source_retrieval_times.json',{}).get(rel,{})
    return record.get('retrieved_at',NA) if record.get('sha256')==file_hash(path) else NA

def review_inputs_approved(path,approval,snapshot_paths):
    """Review applies to exact extraction and primary-source bytes, not a filename."""
    if not path.is_file() or approval.get('sha256')!=file_hash(path):
        return False
    expected=approval.get('source_hashes',{})
    return bool(snapshot_paths) and all(
        (ROOT/rel).is_file() and expected.get(rel)==file_hash(ROOT/rel)
        for rel in set(snapshot_paths)
    )

def approved_curations():
    path=ROOT/'data/curation_overrides_0.2.json'
    approval=read_json(ROOT/'data/reviewed_curations.json',{})
    curations=read_json(path,{})
    snapshots=source_snapshot_paths(curations.get('sources',{}).values())
    for lane in ('cytokine','genomic','proteomic'):
        packpath=ROOT/'research'/lane/'pack.json'
        pack=read_json(packpath,{})
        snapshots.extend([str(packpath.relative_to(ROOT))]+source_snapshot_paths(pack.get('sources',[])))
    if review_inputs_approved(path,approval,snapshots):
        return curations
    return {}

def legacy_pack_unchanged(lane,packpath,pack):
    if not PREVIOUS_VERSION: return False
    previous=ROOT/'releases'/PREVIOUS_VERSION
    manifest=read_json(previous/'manifest.json',{})
    redactions=verified_security_redactions(previous)
    paths=[str(packpath.relative_to(ROOT))]+source_snapshot_paths(pack['sources'])
    for rel in paths:
        expected=redactions.get(rel,{}).get('sanitized_sha256',manifest.get('files',{}).get(rel))
        if not (ROOT/rel).is_file() or file_hash(ROOT/rel)!=expected:
            return False
    return True
...
def build_reference_entities(db):
    ...
    for f in sorted((ROOT/'research/references').glob('uniprot_*.json')):
        ...
        sid='UP_'+acc
        insert(db,'source_registry',dict(source_id=sid,title=symbol+' human protein identity',authors='UniProt Consortium',year=NA,url='https://rest.uniprot.org/uniprotkb/'+acc+'.json',doi=NA,pmid=NA,source_type='resource',version=str(rec.get('entryAudit',{}).get('entryVersion',NA)),retrieved_at=recorded_retrieval_time(f),access_level='API_JSON_identity',snapshot_path=str(f.relative_to(ROOT)),snapshot_sha256=file_hash(f),species='Homo sapiens',notes='Identity snapshot only; functional annotations are not imported as assertions. Retrieval timestamp is local snapshot write time.',ingestion_status='identity_only',duplicate_of=NA))
...
def build():
    if (RELEASE/'manifest.json').exists():
        raise ValueError('A frozen release is immutable. Choose a new release version.')
    missing=[lane for lane in PACK_LANES if not (ROOT/'research'/lane/'pack.json').exists()]
    if missing:
        raise ValueError('Required evidence packs are not ready: '+', '.join(missing))
    credential_files=[str(p.relative_to(ROOT)) for p,_,_ in credential_metadata_findings((ROOT/'research').rglob('*'))]
    if credential_files:
        raise ValueError('Source snapshots contain credential metadata; sanitize before compiling: '+dump(credential_files))
    ...
    all_raw=[]
    reviewed_packs=read_json(ROOT/'data/reviewed_expansion_packs.json',{})
    curations=approved_curations()
    for lane in PACK_LANES:
        ...
        pack=read_json(packpath)
        legacy=lane in ('cytokine','genomic','proteomic')
        independent_review_passed=legacy_pack_unchanged(lane,packpath,pack) if legacy else review_inputs_approved(packpath,reviewed_packs.get(lane,{}),source_snapshot_paths(pack['sources']))
        if not legacy:
            screening=read_json(ROOT/'research'/lane/'screening.json',[])
            candidates=screening.get('candidates',screening.get('screening',[])) if isinstance(screening,dict) else screening
            if pack.get('assertions') and (not candidates or not pack.get('searches')):
                raise ValueError('Expansion evidence requires registered candidate screening: '+lane)
            for source in pack['sources']:
                assertions=[a for a in pack['assertions'] if a['source_id']==source['source_id']]
                if not assertions: continue
                allowed={'include'} if any(a.get('qc_status')=='reviewed_extraction' for a in assertions) else {'include','await_full_text'}
                matches=[c for c in candidates if c.get('decision') in allowed and any(is_known(source.get(k)) and source.get(k)==c.get(k) for k in ('source_id','doi','pmid','url'))]
                if not matches:
                    raise ValueError('Extracted source lacks a corresponding eligible screening decision: '+source['source_id'])
        for source in pack['sources']:
            source={**source,**curations.get('sources',{}).get(source['source_id'],{})}
        ...
        for c in pack['cohorts']:
            c={**c,**curations.get('cohorts',{}).get(c['cohort_id'],{})}
            insert(db,'cohorts',c)
        ...
        for a in pack['assertions']:
            ...
            a.update(legacy_annotation(a,curations.get('assertions',{}).get(a['assertion_id'])) if legacy else explicit_annotation(a.get('scoring_annotation',{})))
            species=a.get('species',NA)
            subject=ensure_entity(db,a['subject'],a['subject_type'],a['subject_layer'],a.get('subject_species',species))
            obj=ensure_entity(db,a['object'],a['object_type'],a['object_layer'],a.get('object_species',species))
            ...
            status='contested' if is_contested(a,cid,blockers,cohort_map) else 'needs_independent_review' if not independent_review_passed else 'reviewed' if a.get('qc_status')=='reviewed_extraction' else 'needs_full_text'
            ...
            if a.get('quantitative_result'):
                insert(db,'quantitative_results',dict(result_id='QUANT_'+aid,evidence_id=ev,reported_result=a['quantitative_result'],interpretation='Source-reported quantitative observation only; not a calibrated distribution.'))
            ...
            if m in ('protein_abundance','secreted_protein','protein_activity','receptor_signaling','binding'):
        ...
        if isinstance(screening,dict):
            screening=screening.get('candidates',screening.get('screening',[]))
        for candidate in screening:
            insert(db,'screening_candidates',dict(candidate,lane=lane))
        for index,g in enumerate(pack.get('gaps',[]),1):
            gap_id=lane+'_'+str(index)
            followup=read_json(ROOT/'data'/('gap_dispositions_'+VERSION+'.json'),{}).get(gap_id)
            description={'original_gap':g,'followup':followup} if followup else g
            insert(db,'coverage_gaps',dict(gap_id=gap_id,lane=lane,description=description,status=followup['status'] if followup else 'open'))
    ...
    for s in read_json(ROOT/'research/references/search_registry.json',[]):
        insert(db,'search_log',s)
    for attempt in read_json(ROOT/'research/fulltext_followup/retrieval.json',[]):
        insert(db,'search_log',dict(search_id='FULLTEXT_'+attempt['pmcid'],lane='fulltext_followup',query=attempt['url'],database='Europe PMC fullTextXML',searched_at=attempt['retrieved_at'],result_source_ids=NA,screening_notes='Full-text upgrade attempt: '+attempt['access_level']+'; '+attempt.get('error','retrieved'),snapshot_path='research/fulltext_followup/retrieval.json',retrieval_limit='single_article'))
    compile_priors(db)
    compile_paths(db)
    compile_release_changes(db)
    ...
    instruction_paths=[ROOT/'README.md',ROOT/'protocol/PROTOCOL.md',ROOT/'protocol/AMENDMENT_0.2.md',ROOT/'research/EXTRACTION_CONTRACT.md',ROOT/'research/EXTRACTION_CONTRACT_0.2.md',ROOT/'provenance/USER_REQUEST.md']+sorted((ROOT/'provenance/skill_snapshots').glob('*.md'))
    for p in instruction_paths:
        instruction_version=VERSION if p.name in ('AMENDMENT_0.2.md','EXTRACTION_CONTRACT_0.2.md') else '0.1.0' if p.name in ('README.md','PROTOCOL.md','EXTRACTION_CONTRACT.md') else NA
        insert(db,'instruction_versions',dict(instruction_id=uid('INS',str(p.relative_to(ROOT))),version=instruction_version,path=str(p.relative_to(ROOT)),sha256=file_hash(p),scope='governing_specification' if p.name=='README.md' else 'operational_instruction'))
    session=read_json(ROOT/'provenance'/('session_'+VERSION+'.json'),{})
    ...
    insert(db,'agent_runs',dict(run_id='RUN_COORDINATOR_'+VERSION,agent_role='coordinator',timestamp_start=session.get('timestamp_start',NA),timestamp_end=session.get('timestamp_end',NA),timezone='America/New_York',LLM_provider='OpenAI',LLM_model='GPT-6',model_snapshot_if_available=NA,reasoning_mode=NA,protocol_version=VERSION,agent_prompt_version=VERSION,instruction_bundle_hash=digest([file_hash(p) for p in instruction_paths]),source_versions='source_registry.csv',retrieval_queries='search_log.csv',retrieval_dates='source_registry.csv',source_hashes='artifact_hashes.csv',code_commit=NA,environment_hash=digest({'python':platform.python_version(),'platform':platform.platform()}),output_hash=semantic_output_hash,critic_run_ids=[r[0] for r in db.execute('SELECT critic_run_id FROM critic_runs')],human_edits=NA,original_metadata=session))
...
def compare_records(old,new):
    for rid in sorted(set(old)|set(new)):
        if rid not in old:
            yield rid,'added',None,new[rid]
        elif rid not in new:
            yield rid,'removed',old[rid],None
        elif old[rid]!=new[rid]:
            yield rid,'modified',old[rid],new[rid]

def compile_release_changes(db):
    path=ROOT/'releases'/PREVIOUS_VERSION/'immune_prior.sqlite' if PREVIOUS_VERSION else None
    if path is not None:
        if not path.exists() or manifest_mismatches(path.parent):
            raise ValueError('Previous frozen release is missing or failed hash verification')
    previous=sqlite3.connect('file:'+str(path)+'?mode=ro',uri=True) if path and path.exists() else None
    if previous: previous.row_factory=sqlite3.Row
    for table,key in [('edge_assertions','assertion_id'),('evidence_instances','evidence_id'),('prior_parameters','prior_id'),('source_registry','source_id')]:
        old={r[key]:dict(r) for r in previous.execute('SELECT * FROM '+table)} if previous else {}
        new={r[key]:dict(r) for r in db.execute('SELECT * FROM '+table)}
        for rid,kind,before,after in compare_records(old,new):
            insert(db,'release_changes',dict(change_id=uid('DELTA',[VERSION,table,rid]),record_type=table,record_id=rid,previous_version=PREVIOUS_VERSION or NA,new_version=VERSION,change_type=kind,old_record=before,new_record=after))
            if table=='prior_parameters' and after is not None:
                insert(db,'prior_changes',dict(change_id='CHANGE_'+rid,prior_id=rid,previous_version=PREVIOUS_VERSION or NA,new_version=VERSION,previous_parameters=before['parameters'] if before else NA,new_parameters=after['parameters'],reason=kind+' evidence-derived prior; compared against preserved previous release.'))
...
def input_files():
    ...
    if PREVIOUS_VERSION:
        previous=ROOT/'releases'/PREVIOUS_VERSION
        # Historical rebuilds also verify the predecessor. Preserve the complete
        # dependency closure, including its archived inputs, inside this snapshot.
        manifest=read_json(previous/'manifest.json',{})
        paths.extend(p for p in [previous/'manifest.json',previous/'security_redactions.json']+[manifest_path(rel,previous) for rel in manifest.get('files',{})] if p.is_file())
...
def validate(db):
    ...
    for r in db.execute('SELECT * FROM screening_candidates'):
        if r['decision'] not in ('include','exclude','await_full_text'):
            errors.append('Unknown screening decision: '+r['candidate_id'])
        if not is_known(r['reason']) or not is_known(r['url']):
            errors.append('Missing screening reason/source: '+r['candidate_id'])
        if not db.execute('SELECT 1 FROM search_log WHERE search_id=?',(r['query_id'],)).fetchone():
            errors.append('Unregistered candidate query: '+r['candidate_id'])
    ...
    for r in db.execute('SELECT a.*,s.layer sl,o.layer ol FROM edge_assertions a JOIN entities s ON s.entity_id=a.subject_id JOIN entities o ON o.entity_id=a.object_id'):
        expected={'RNA_abundance':{'T'},'splicing':{'T'},'direct_transcriptional_regulation':{'T'},'protein_abundance':{'P'},'secreted_protein':{'P'},'protein_activity':{'P'},'receptor_signaling':{'P','C'},'cellular_phenotype':{'F'},'binding':{'P'}}
    ...
    if {'IL-8','IL-8 high range'}<=alias.keys() and alias['IL-8']['entity_id']!=alias['IL-8 high range']['entity_id']:
        errors.append('CXCL8 alias identity mismatch')
    if {'IL-12','IL-12p70'}<=alias.keys() and alias['IL-12']['entity_id']==alias['IL-12p70']['entity_id']:
        errors.append('Ambiguous IL-12 assay silently merged')
    return errors

def report(db,qc):
    counts=qc['counts']
    text='# Immune-system evidence release\n\n'
    text+=f"Release {VERSION} contains {counts['edge_assertions']} evidence assertions, {counts['cohorts']} cohort/context records and {counts['source_registry']} source records (including reference resources and identity snapshots). It is a bounded evidence corpus, not an exhaustive systematic review. Human adjudication and probability calibration remain outstanding.\n\n"
    ...
    text+='## Changes from the previous release\n\n| Record type | Change | Count |\n|---|---|---|\n'
    for r in db.execute('SELECT record_type,change_type,COUNT(*) n FROM release_changes GROUP BY record_type,change_type ORDER BY record_type,change_type'):
    ...
    text+='## Evidence by review state\n\n| Evidence lane | Review state | Assertions |\n|---|---|---|\n'
    for r in db.execute('SELECT e.extraction_lane,a.status,COUNT(*) n FROM edge_assertions a JOIN evidence_instances e USING(assertion_id) GROUP BY e.extraction_lane,a.status ORDER BY e.extraction_lane,a.status'):
        text+=f"| {r['extraction_lane']} | {r['status']} | {r['n']} |\n"
    text+='\n## Historical assay aliases\n\nThese labels are retained for provenance and do not define the all-cytokine project scope or its coverage denominator. Counts describe protein-involving assertions, including interventions; they do not certify direct assay-analyte measurements.\n\n| Original label | Reviewed protein-involving assertions | Mapping status |\n|---|---|---|\n'
    for r in db.execute('SELECT * FROM assay_coverage ORDER BY original_label'):
        text+=f"| {r['original_label']} | {r['reviewed_protein_assertion_count']} | {r['mapping_status']} |\n"
    text+='\n'
...
def freeze():
    if (RELEASE/'manifest.json').exists():
        raise ValueError('A frozen manifest is immutable. Use verify or choose a new release.')
    ...
    files=input_files()+[p for p in RELEASE.iterdir() if p.is_file() and p.name!='manifest.json']+[ROOT/'data/schema.sql',ROOT/'data/DATA_DICTIONARY.md']
    manifest={'release':VERSION,'previous_release':PREVIOUS_VERSION,'protocol_version':VERSION,'scientific_status':'bounded_evidence_expansion','manifest_excludes_itself':True,'hash_algorithm':'sha256','files':{str(p.relative_to(ROOT)):file_hash(p) for p in sorted(set(files))}}
...
def manifest_path(rel,release= None):
    release=release or RELEASE
    prefix=str(release.relative_to(ROOT))+'/'
    return ROOT/rel if rel.startswith(prefix) else release/'input_snapshot'/rel

def snapshot_inputs(manifest):
    for rel,expected in manifest['files'].items():
        target=manifest_path(rel)
        source=ROOT/rel
        if target==source: continue
        if target.exists() and file_hash(target)!=expected:
            raise ValueError('Conflicting frozen input: '+rel)
        target.parent.mkdir(parents=True,exist_ok=True)
        if not target.exists(): shutil.copy2(source,target)
        if file_hash(target)!=expected: raise ValueError('Snapshot copy failed: '+rel)

def manifest_mismatches(release):
    m=read_json(release/'manifest.json')
    if m is None: return ['missing_manifest']
    bad=[]
    redactions=verified_security_redactions(release)
    for rel,expected in m['files'].items():
        p=manifest_path(rel,release)
        replacement=redactions.get(rel,{}).get('sanitized_sha256',expected)
        if not p.is_file() or file_hash(p)!=replacement:
            bad.append(rel)
    return bad

def verified_security_redactions(release):
    """Security exceptions preserve the historical digest and are visibly reported."""
    path=release/'security_redactions.json'
    ledger=read_json(path,{})
    manifest=read_json(release/'manifest.json',{})
    if not ledger: return {}
    if ledger.get('original_manifest_sha256')!=file_hash(release/'manifest.json'):
        raise ValueError('Security-redaction ledger does not match the original manifest')
    rows=ledger.get('redactions',{})
    for rel,row in rows.items():
        if not rel.startswith('research/') or manifest.get('files',{}).get(rel)!=row.get('original_sha256') or not re.fullmatch('[0-9a-f]{64}',row.get('sanitized_sha256','')):
            raise ValueError('Invalid security-redaction record: '+rel)
    return rows

def verify():
    m=read_json(RELEASE/'manifest.json')
    bad=manifest_mismatches(RELEASE)
    redactions=verified_security_redactions(RELEASE) if m else {}
    print(json.dumps({'checked':len(m['files']) if m else 0,'mismatches':bad,'documented_security_redactions':sorted(redactions),'exact_original_bytes_preserved':not bool(redactions)},indent=2))
    return not bad

def main():
    global VERSION,PREVIOUS_VERSION,RELEASE
    ...
    if not re.fullmatch(r'\d+\.\d+\.\d+',args.release):
        parser.error('Release must have the form major.minor.patch')
    VERSION=args.release;RELEASE=ROOT/'releases'/VERSION
    PREVIOUS_VERSION=read_json(ROOT/'data/release_config.json',{}).get(VERSION,{}).get('previous_release',None)
