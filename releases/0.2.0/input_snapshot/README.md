...
Release 0.2.0 expands the initial 72-assertion corpus with primary cytokine and chemokine evidence and reviewed corrections to all eight inherited assertions that needed full text. It includes normalized SQLite/CSV tables, source snapshots, independent review, explicit prior interfaces, and reproducible release lineage.

Start with [the current release report](reports/RELEASE_0.2.0.md), [RUNNING.md](RUNNING.md), and [the item-by-item project status](reports/PROJECT_STATUS.md). The latter tracks all 107 original checklist requirements and distinguishes implemented infrastructure from remaining scientific coverage. Human adjudication, empirical calibration and broader systematic review remain open. The original specification is preserved in [protocol/README.original.md](protocol/README.original.md); the first-release narrative remains in [the initial research synthesis](reports/RESEARCH_SYNTHESIS.md).

## Project status

**Protocol concept:** Greenfield, multi-omic systematic evidence review
**Current protocol version:** v0.2.0 (original protocol plus coverage/release amendment)
...
# 11. Cytokine priority coverage set
...
Normalize aliases while preserving original assay labels.

Examples:

| Assay     | Canonical entity |
| --------- | ---------------- |
| GM-CSF    | CSF2             |
| TNF-β     | LTA              |
| IL-8      | CXCL8            |
...
IL-8 and IL-8 high-range should share a biological entity but retain different assay-channel provenance.

IL-12 and IL-12p70 should remain distinct assay concepts until the measurement platform is verified.

VEGF identity should likewise be resolved against assay documentation.
