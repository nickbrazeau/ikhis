"""Explicit reviewed initial scoring rules; no automatic direction classifier."""
import json
SUPPORTS=set('''
...
SCORING_KEYS=('tested_hypothesis','evidence_relation','null_adequacy','rationale')
def explicit_annotation(annotation):
    return {k:annotation.get(k,'unavailable') for k in SCORING_KEYS}
def legacy_annotation(assertion,curation=None):
    a=assertion;aid=a['assertion_id']
    if curation is not None:
        return explicit_annotation(curation.get('scoring_annotation',{}))
    return {
